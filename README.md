...
On the first day of each month, a scheduled workflow searches a 45-day overlap in PubMed and ClinicalTrials.gov, rechecks tracked identifiers, and opens a review issue. Children and adolescents are a dedicated report section covering both new PubMed candidates and newly updated trial records. If no child-specific finding has completed review, the public section says so rather than substituting adult-only evidence. Cancer is not a report section, although cancer-related evidence remains eligible when it is relevant to lymphoedema. The report also includes watchlists for arm/upper-limb and trunk/chest/abdominal-wall research, plus French-, German-, Chinese-, and Japanese-language PubMed candidates. A durable deferred-candidates section records relevant evidence that is waiting for a specific appraisal or revisit trigger. Named sections, watchlists, and deferrals organise discovery and presentation; they never restrict eligible research areas or create separate evidence feeds. Relevant work remains eligible even when it does not fit an existing heading, and every candidate uses the same editorial threshold. Automation discovers candidates only; it never writes or publishes medical summaries.
